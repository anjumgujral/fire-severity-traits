...
XFT_filtered_traits <- XFT_traits %>%
  select(uniqueID, binomial_gnr, Plant.organ, Developmental.stage, P50)

XFT_filtered_traits$binomial_gnr <- gsub(" ", "_", XFT_filtered_traits$binomial_gnr)

XFT_filtered_traits <- XFT_filtered_traits %>%
  rename(SpeciesName = binomial_gnr)
...
TRY_filtered_traits <- filtered_trait_data %>%
  select(SpeciesName, TraitID, TraitName, DataName, OriglName, OrigValueStr, OrigUnitStr, StdValue, UnitName, ObservationID, ObsDataID, Dataset, Reference)
...
merged_trait_data <- <- TRY_filtered_traits %>%
  select(SpeciesName, OriglName, StdValue, UnitName, Source) %>%
  left_join(
    XFT_filtered_traits %>%
      select(SpeciesName, P50, Source),
    by = "SpeciesName",
    suffix = c("_TRY", "_XFT")
  )
