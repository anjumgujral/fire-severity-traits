...
# plots with at least 1 fire for disturbance code 1 (classified as most important disturbance at the time of resampling)

# plots with at least 1 fire in any disturbance classificiation (varying importance)
...
# filter CA burned plots extract just plots that burned in the mixed-conifer forest type
forest_type_371 <- CA_fire_only %>%
  filter(FORTYPCD == 371)

# ~250 in ground fire and ~50 in canopy fire in CA mixed conifer
...
# expanding to similar mixed-conifer forest types == 300+ plots in ground fire, and ~100 plots canopy fire  
...
# forest_type_combined is our key dataframe with unique PLT_CN and their disturbance codes originally from CA_COND
CA_TREE_fire_only <- CA_TREE %>%
  filter(PLT_CN %in% forest_type_combined$PLT_CN)
#467 plots burned in combined mixed conifer forest types

# now filter CA_TREE data by burned plots in our combined forest types
# I think this is circular but double check 
CA_TREE_fire_only <- CA_TREE %>%
...
# lets add some more mixed conifer states and run through the filtering process again
...
CA_forest_type_combined <- CA_fire_only %>%
  filter(FORTYPCD %in% c(371, 201, 221, 222, 224, 225, 261, 922))
...
#plot by condition
```{r}
# take forest type combined dataframes (which are COND tables filtered for our disturbance and forest types) and extract the unique CN codes from forest_type_combined tables and filter PLOT tables (with all revisits) for the select CN's 

# disturbance history
CA_disturbance_history <-  CA_forest_type_combined %>%
  dplyr::select(
    PLT_CN,
    DSTRBCD1, DSTRBYR1,
    DSTRBCD2, DSTRBYR2,
...
CA_PLOT_by_COND <- CA %>%
  left_join(CA_disturbance_history, by = c("CN" = "PLT_CN"))
# this retains multiple rows for a PLT_CN due to multiple rows for PLT_CN x CONDID combo from CA_COND
...
revisit_table <- CA %>%
  ...
  dplyr::summarise(
    n_visits = dplyr::n(),
    first_year = min(INVYR),
    last_year = max(INVYR),
    .groups = "drop"
  )

# 38021 plots have differing DSTRBCD between rows of the same PLT_CN in COND table

#variation in COND rows per plot may be ecologically meaningful


#CA_PLOT_by_COND <- CA_PLOT_by_COND %>%
  #dplyr::left_join(revisit_table, by = "PLOT")


# lets simplify by taking the dominant condition 
# CONDPROP_UNADJ = proportion of the plot occupied by that condition (ranges from # 0–1, sums to ~1 across conditions in a plot)

# the dominant condition = the largest CONDPROP_UNADJ for a given PLT_CN.

CA_cond_main <- CA_cond %>%
  group_by(PLT_CN) %>%
  slice_max(order_by = CONDPROP_UNADJ, n = 1, with_ties = FALSE) %>%
  ungroup()

#CA_plot_level <- CA %>%
  #left_join(CA_cond_main, by = c("CN" = "PLT_CN"))
...
# filter plots with a fire as primary disturbance and keep the ones that have a year recorded for disturbance year
...
# get all inventories for the above filtered plots
CA_all_fire_plot_inventories <- CA_cond %>%
  filter(PLT_CN %in% CA_fire_only_yr$PLT_CN)

# join DSTRBCD1 and DSTRBYR1 to all inventories for filtered plots
CA_all_fire_plot_inventories <- CA_all_fire_plot_inventories %>%
  left_join(CA_fire_only_yr, by = "PLT_CN")

# Flag pre- and post-fire records
CA_fire_only_pre_post_fire_surveys <- CA_all_fire_plot_inventories %>%
  mutate(
    pre_fire = INVYR.x < DSTRBYR1.x,
    post_fire = INVYR.x > DSTRBYR1.x
  )

# keep plots with both pre and post fire surveys
CA_fire_only_pre_post_fire_surveys2 <- CA_fire_only_pre_post_fire_surveys %>%
  group_by(PLT_CN) %>%
  filter(any(pre_fire) & any(post_fire)) %>%
  ungroup()
# only 8 of the 400 plots in CA have both pre and post surveys?

# check for the other states
OR_fire_only_yr <- OR_cond %>%
  dplyr::filter(DSTRBCD1 %in% c(30, 31, 32), !is.na(DSTRBYR1)) %>%
  dplyr::select(PLT_CN, DSTRBYR1, DSTRBCD1, INVYR) %>%
  dplyr::distinct()

# get all inventories for the above filtered plots
OR_all_fire_plot_inventories <- OR_cond %>%
  filter(PLT_CN %in% OR_fire_only_yr$PLT_CN)

# join DSTRBCD1 and DSTRBYR1 to all inventories for filtered plots
OR_all_fire_plot_inventories <- OR_all_fire_plot_inventories %>%
  left_join(OR_fire_only_yr, by = "PLT_CN")

# Flag pre- and post-fire records
OR_fire_only_pre_post_fire_surveys <- OR_all_fire_plot_inventories %>%
  mutate(
    pre_fire = INVYR.x < DSTRBYR1.x,
    post_fire = INVYR.x > DSTRBYR1.x
  )
...
# keep plots with both pre and post fire surveys
OR_fire_only_pre_post_fire_surveys2 <- OR_fire_only_pre_post_fire_surveys %>%
  group_by(PLT_CN) %>%
  filter(any(pre_fire) & any(post_fire)) %>%
  ungroup()

# 44 burned plots with pre and post fire surveys in OR out of a total of 603 burned plots in OR 

WA_fire_only_yr <- WA_cond %>%
  dplyr::filter(DSTRBCD1 %in% c(30, 31, 32), !is.na(DSTRBYR1)) %>%
  dplyr::select(PLT_CN, DSTRBYR1, DSTRBCD1, INVYR) %>%
  dplyr::distinct()

# get all inventories for the above filtered plots
WA_all_fire_plot_inventories <- WA_cond %>%
  filter(PLT_CN %in% WA_fire_only_yr$PLT_CN)

# join DSTRBCD1 and DSTRBYR1 to all inventories for filtered plots
WA_all_fire_plot_inventories <- WA_all_fire_plot_inventories %>%
  left_join(WA_fire_only_yr, by = "PLT_CN")

# Flag pre- and post-fire records
WA_fire_only_pre_post_fire_surveys <- WA_all_fire_plot_inventories %>%
  mutate(
    pre_fire = INVYR.x < DSTRBYR1.x,
    post_fire = INVYR.x > DSTRBYR1.x
  )
...
# keep plots with both pre and post fire surveys
WA_fire_only_pre_post_fire_surveys2 <- WA_fire_only_pre_post_fire_surveys %>%
  group_by(PLT_CN) %>%
  filter(any(pre_fire) & any(post_fire)) %>%
  ungroup()

# only 18 of the 235 plots in WA have both pre and post surveys
...
CA_PLOT_fire_only <- CA %>%
  filter(CN %in% CA_forest_type_combined$PLT_CN)

temp <- CA_PLOT_fire_only %>%
    group_by(STATECD, UNITCD, COUNTYCD, PLOT) %>%
    summarise(n_surveys = n())
min(temp$n_surveys)

# CA fire only plots have a max of 2 surveys

# CA_cond has the disturbance info but not revisit info, CA_PLOT has all the  revisits for each unique plot
# combine disturbance info from CA_cond to CA_PLOT

CA_PLOT_fire_only_revists <-  CA %>%
  left_join(
    CA_COND %>%
      select(PLT_CN, DSTRBCD1, DSTRBYR1),
    by = c("CN" = "PLT_CN")
  )

WA_PLOT_fire_only <- WA_PLOT %>%
...
```{r}
max(xtabs(~TREE, CA_TREE))
CA_TREE %>% group_by(PLT_CN) %>% summarize(nrevist = length(unique(INVYR)))

temp <- (CA_TREE %>% group_by(PLT_CN) %>% summarize(nrevist = length(unique(INVYR))))
max(temp$nrevist)

# to find out the number of revisits for a unique plot, you have to use the CA_PLOT dataframe, CA_cond will just have one inventory for each plot because it is just telling us the conditions of that plot, mulitple rows for a plot in CA_cond due to different conditions
