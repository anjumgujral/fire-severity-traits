...
# and then filter CA burned plots extract just plots that burned in focal forest types including mixed conifer, douglas-fir, ponderosa pine,
...
# 3801 plots have differing DSTRBCD between rows of the same PLT_CN in COND table
...
```{r}
# CA_PLOT_by_COND now has multiple revisits (CN values) per PLOT with a single  disturbance profile for each plot  (using the dominant disturbance) 

# determine the number of pre and post fire surveys using any/all disturbance years

# first flag pre and post fire surveys
CA_pre_post <- CA_PLOT_by_COND %>%
  mutate(
    pre_fire = if_any(
      c(DSTRBYR1, DSTRBYR2, DSTRBYR3),
      ~ !is.na(.x) & INVYR < .x
    ),
    post_fire = if_any(
      c(DSTRBYR1, DSTRBYR2, DSTRBYR3),
...
CA_pre_post_plots <- CA_pre_post %>%
  group_by(PLOT) %>%
  summarise(
    has_pre = any(pre_fire, na.rm = TRUE),
    has_post = any(post_fire, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  filter(has_pre & has_post)

# 30 plots with pre and post fire inventories 
...
# 76 plots with pre and post fire surveys
```
...
# 44 plots with pre and post fire inventories 
